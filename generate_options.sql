...
Begin
  ...
  BEGIN TRY 
	...
	if object_id('Temp.PRPHOptions') is not null
	drop table Temp.PRPHOptions

	if object_id('Temp.FullProvisionOption') is not null
	drop table Temp.FullProvisionOption

	if object_id('Temp.FinalProvisionOptions') is not null
	drop table Temp.FinalProvisionOptions
 ...
 --                           End
	...
	into Temp.FullProvisionOption
	from Temp.PRPHOptions
	...
	into Temp.FinalProvisionOptions
	from Temp.PRPHOptions A
	inner join
		Temp.FullProvisionOption B
	on  A.pr_id					=  b.pr_id
